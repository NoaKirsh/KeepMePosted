...
class TestSummarizerWithMockedAPI:
    ...
    @pytest.mark.asyncio
    async def test_analyze_articles_success(
        self, summarizer, sample_articles, mock_gemini_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_articles_uses_correct_config(
        self, summarizer, sample_articles, mock_gemini_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_articles_lazy_loads_and_caches_client(
        self, summarizer, sample_articles, mock_gemini_response
    ):
        ...
        with patch(
            "agents.summarizer.get_google_ai_client", return_value=mock_genai
        ) as mock_get_client:
            # First call
            await summarizer.analyze_articles(sample_articles)
